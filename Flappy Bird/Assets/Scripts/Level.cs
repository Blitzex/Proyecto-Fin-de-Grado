...
public class Level : MonoBehaviour { 
    ...
    //Tamaño y posiciones de los elementos del juego.

    private const float CAMERA_ORTHO_SIZE = 50f; 
    private const float PIPE_WIDTH = 7.8f;
    private const float PIPE_HEAD_HEIGHT = 3.75f;
    private const float PIPE_MOVE_SPEED = 30f;
    private const float PIPE_DESTROY_X_POSITION = -100f; //posición en la que se destruyen las tuberías.
    ...
    public enum Difficulty { //Dificultades.
        Easy,
        Medium,
        Hard,
        Impossible,
    ...
    private void HandlePipeSpawning() { //Crea las tuberías de manera aleatoria con un rango de tamaño cada x segundos.
    ...
    //Hace que las tuberías se vayan desplanzando de forma constante hacia la izquierda dando la sensación de movimiento.
    ...
    private void HandlePipeMovement() { 
        for (int i=0; i<pipeList.Count; i++) {
            ...
            if (pipe.GetXPosition() < PIPE_DESTROY_X_POSITION) { //destruye la tubería al salir de rango.
                // Destroy Pipe
                pipe.DestroySelf();
    ...
    private void CreatePipe(float height, float xPosition, bool createBottom) { //Crea las tuberias con una altura y una posición.
        // Set up Pipe Head
